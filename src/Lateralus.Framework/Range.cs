...
{
    public static Range<T> Create<T>(T from, T to) where T : IComparable<T> => new Range<T>(from, to);
}
    ...
    where T : IComparable<T>
{
    ...
    public bool IsInRangeInclusive(Range<T> range) => IsInRangeInclusive(range.From) && IsInRangeInclusive(range.To);

    public bool IsInRangeExclusive(Range<T> range) => IsInRangeExclusive(range.From) && IsInRangeExclusive(range.To);

    public bool IsInRangeLowerInclusive(Range<T> range) => IsInRangeLowerInclusive(range.From) && IsInRangeLowerInclusive(range.To);

    public bool IsInRangeUpperInclusive(Range<T> range) => IsInRangeUpperInclusive(range.From) && IsInRangeUpperInclusive(range.To);
