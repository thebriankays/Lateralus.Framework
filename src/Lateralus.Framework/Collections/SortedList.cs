...
public sealed class SortedList<T> : ICollection<T>, ICollection, IReadOnlyList<T>
{
    ...
    public void CopyTo(int index, Span<T> array, int count) => _items.AsSpan(index, count).CopyTo(array);

    public void CopyTo(int index, Memory<T> array, int count) => _items.AsMemory(index, count).CopyTo(array);
    ...
    public int BinarySearch(T item) => Array.BinarySearch(_items, index: 0, length: Count, item, _comparer);
    ...
    public ReadOnlySpan<T> UnsafeAsReadOnlySpan() => _items.AsSpan(0, Count);
