...
{
    public static int TryReadAll(this Stream stream, byte[] buffer, int offset, int count) => TryReadAll(stream, buffer.AsSpan(offset, count));
    ...
    public static Task<int> TryReadAllAsync(this Stream stream, byte[] buffer, int offset, int count, CancellationToken cancellationToken = default) => TryReadAllAsync(stream, buffer.AsMemory(offset, count), cancellationToken);
